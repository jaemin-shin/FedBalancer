    ...
    for fb_p in fb_ps:
        for lr_stepsize in lr_stepsizes:
            for ddl_stepsize in ddl_stepsizes:
                ...
                if os.path.exists('configs/paper_experiments/220228_fedbalancer/shakespeare/shakespeare_fb_p'+str_fb_p+'_as'+str_as+'_lss'+str_lr+'_dss'+str_ddl+'_'+postfix+'.cfg.log'):
                    file = open('configs/paper_experiments/220228_fedbalancer/shakespeare/shakespeare_fb_p'+str_fb_p+'_as'+str_as+'_lss'+str_lr+'_dss'+str_ddl+'_'+postfix+'.cfg.log')
                    ...
                    if is_final_appeared:
                        print('configs/paper_experiments/220228_fedbalancer/shakespeare/shakespeare_fb_p'+str_fb_p+'_as'+str_as+'_lss'+str_lr+'_dss'+str_ddl+'_'+postfix+'.cfg.log is done!')
                        continue
                    else:
                        if process_count >= process_count_start and process_count < process_count_end:
                            new_config_file = open('configs/paper_experiments/220228_fedbalancer/shakespeare/shakespeare_fb_p'+str_fb_p+'_as'+str_as+'_lss'+str_lr+'_dss'+str_ddl+'_'+postfix+'.cfg', 'w')
                            for line in lines:
                            ...
                            os.system('CUDA_VISIBLE_DEVICES='+str(7-(process_count//1))+' python main.py --config=configs/paper_experiments/220228_fedbalancer/shakespeare/shakespeare_fb_p'+str_fb_p+'_as'+str_as+'_lss'+str_lr+'_dss'+str_ddl+'_'+postfix+'.cfg &')
